...
await withPage(async (page) => {
  ...
  await page.evaluate(() => {
    ...
    new PerformanceObserver((l) => {
      for (const e of l.getEntries()) window.__navLongTasks.push(Math.round(e.duration))
  ...
  results.runs.docToDocNav = {
    navCommitMs: navCommit,
    settledMs: settled,
...
await withPage(async (page) => {
  ...
  if (!editor) {
    results.runs.typing = {note: 'no editable comment box found'}
    return
  }
  ...
  await page.evaluate(() => {
    window.__typeLongTasks = []
    new PerformanceObserver((l) => {
      for (const e of l.getEntries()) window.__typeLongTasks.push(Math.round(e.duration))
    }).observe({type: 'longtask'})
  })
  const latencies = []
