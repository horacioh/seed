...
const IPFS: React.FC = () => {
  ...
  return <div className={stylex.props(styles_2.se7814c81, styles_2.s5574c491, styles_2.s1aa17).className || ""}>
      <Title buttons={<>
            <CopyTextButton text={`ipfs://${cid}`} />
            <DownloadButton url={`${apiHost}/ipfs/${cid}`} />
          </>} title={`ipfs://${cid}`} />
      {revisedData && <div className={stylex.props(styles.s33458e).className || ''}>
