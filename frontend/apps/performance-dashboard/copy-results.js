...
function generateElectronIndex(electronDestDir) {
  try {
    ...
    const perfFiles = files.filter((file) => file.startsWith('perf-metrics-') && file.endsWith('.json'))
...
try {
  ...
  if (fs.existsSync(electronSourceDir)) {
    ...
    console.warn(`Electron performance results directory does not exist: ${electronSourceDir}`)
  }
  ...
  if (fs.existsSync(webSourceDir)) {
    ...
    console.warn(`Web performance results directory does not exist: ${webSourceDir}`)
  }
