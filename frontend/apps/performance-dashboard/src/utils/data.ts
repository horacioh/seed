import type {PerformanceReport, WebPerformanceMetric, WebPerformanceReport, WebPerformanceResult} from '../types'

/**
 * Load performance reports from the specified directory
 */
export async function loadPerformanceReports(appType: string = 'electron'): Promise<PerformanceReport[]> {
  try {
    // Determine the correct path based on app type
    const basePath = appType == 'electron' ? '/results/electron' : `/results/${appType}`
 ...
 */
export const loadReportById = async (reportId: string): Promise<PerformanceReport | null> => {
  ...
  try {
    ...
    console.log(`[loadReportById] Attempting to fetch report ${reportId} from: ${url}`)

    const response = await fetch(url)

    console.log(`[loadReportById] Response status for report ${reportId}: ${response.status} ${response.statusText}`)
    ...
    console.log(`[loadReportById] Report data received for ${reportId}:`, reportData)

    // Transform the data to match the expected format
    const scenarios = Object.entries(reportData.metrics).map(([name, metrics]: [string, any]) => ({
      name,
        ...
        .map(([metricName, value]) => {
          ...
          if (metricName === 'cpuUsage') {
            return Object.entries(
              ...
              name: cpuMetricName,
              value: cpuValue,
              unit: cpuMetricName.includes('Percentage') || cpuMetricName === 'percentCPUUsage' ? '%' : '',
            }))
          }
...
export const getTracePath = (reportId: string, traceFile: string): string => {
  ...
  console.log(`[getTracePath] Generated path for report ${reportId}, trace ${traceFile}: ${path}`)
  return path
}
 ...
 */
export const getScreenshotPath = (reportId: string, screenshotFile: string): string => {
  const path = `/results/electron/${reportId}/screenshots/${screenshotFile}`
  console.log(`[getScreenshotPath] Generated path for report ${reportId}, screenshot ${screenshotFile}: ${path}`)
  return path
}

// Load electron performance reports
export async function loadElectronPerformanceReports(): Promise<PerformanceReport[]> {
  try {
    ...
    const fullReports = await Promise.all(
      reports.map(
        async (report: {
          ...
          try {
            const reportResponse = await fetch(`/results/electron/${report.file}`)
    ...
    return fullReports.filter((report): report is PerformanceReport => report !== null)
  } catch (error) {
...
export async function loadWebPerformanceResults(app: 'web' | 'landing'): Promise<WebPerformanceResult[]> {
  try {
    ...
    return uniqueResults.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
  } catch (error) {
...
export function transformWebResult(result: WebPerformanceResult): WebPerformanceReport {
