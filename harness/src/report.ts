...
export async function writeReports(
  ...
  const imageMarkup = await Promise.all(
    screenshots.map(async (relative) => {
      ...
      return `<figure><img src="data:image/png;base64,${data.toString('base64')}" alt="${escapeHtml(
        relative,
      )}"><figcaption>${escapeHtml(relative)}</figcaption></figure>`
    }),
  )
  ...
  .map(
    (step) =>
      `<tr><td>${escapeHtml(step.description ?? step.id)}</td><td>${escapeHtml(step.action)}</td><td>${escapeHtml(
        step.value ?? '',
      )}</td><td class="${step.status === 'pass' ? 'pass' : 'fail'}">${step.status}</td><td>${escapeHtml(
        step.screenshotRef ?? '',
      )}</td></tr>`,
  )
  .join('')}
...
${redactedReport.assertions
  .map(
    (assertion) =>
      ...
      )}</td><td>${escapeHtml(String(assertion.actual ?? assertion.message ?? ''))}</td><td class="${
        assertion.status === 'pass' ? 'pass' : 'fail'
      }">${assertion.status}</td></tr>`,
  )
  .join('')}
</tbody></table><h2>Video</h2>${
    video
      ? `<video controls src="data:video/mp4;base64,${video.toString('base64')}"></video>`
...
export function markdownReport(report: WebTestReport): string {
  ...
  const lines = [
    ...
    `**Assertions:** ${
      report.assertions.filter((assertion) => assertion.status === 'pass').length
    } passed, ${failedAssertions} failed`,
    '',
    '## Reproduction steps',
    ...report.steps.map(
      (step, index) =>
        `${index + 1}. ${escapeMarkdownText(step.description ?? step.action)}${
          step.value !== undefined ? ` — value: ${mdInlineCode(step.value)}` : ''
        }`,
    ),
    '',
    '## Assertions',
    ...report.assertions.map(
      (assertion) =>
        `- **${assertion.status.toUpperCase()}** ${escapeMarkdownText(
          assertion.message ?? assertion.id,
        )} (expected ${mdInlineCode(String(assertion.expected))}, actual ${mdInlineCode(
    ...
    ...(report.findings.length
      ? report.findings.map(
          (finding) =>
            `- **${escapeMarkdownText(finding.severity.toUpperCase())}** ${escapeMarkdownText(
              finding.title,
            )}: ${escapeMarkdownText(finding.actual)}`,
        )
      : ['- None']),
    '',
  ]
...
function renderFinding(finding: Finding): string {
  return `<div class="finding"><strong>${escapeHtml(finding.severity.toUpperCase())}: ${escapeHtml(
    finding.title,
  )}</strong><p>${escapeHtml(finding.actual)}</p></div>`
}
