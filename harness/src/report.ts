...
export async function writeReports(runDir: string, report: WebTestReport): Promise<string> {
  ...
  const failed = report.assertions.filter((assertion) => assertion.status !== "pass").length;
...
<h1>${escapeHtml(report.scenarioTestCase.name)}</h1><p><strong class="${failed ? "fail" : "pass"}">${failed ? "FAIL" : "PASS"}</strong> — ${passed} passed, ${failed} failed</p>
<h2>Steps</h2><table><thead><tr><th>Step</th><th>Action</th><th>Status</th><th>Evidence</th></tr></thead><tbody>
${report.steps.map((step) => `<tr><td>${escapeHtml(step.description ?? step.id)}</td><td>${escapeHtml(step.action)}</td><td class="${step.status === "pass" ? "pass" : "fail"}">${step.status}</td><td>${escapeHtml(step.screenshotRef ?? "")}</td></tr>`).join("")}
...
export function markdownReport(report: WebTestReport): string {
  ...
  const lines = [
    ...
    `**Assertions:** ${report.assertions.filter((assertion) => assertion.status === "pass").length} passed, ${failed} failed`,
    "",
    "## Reproduction steps",
    ...report.steps.map((step, index) => `${index + 1}. ${step.description ?? step.action}`),
    "",
    "## Assertions",
    ...report.assertions.map((assertion) => `- **${assertion.status.toUpperCase()}** ${assertion.message ?? assertion.id} (expected \`${String(assertion.expected)}\`, actual \`${String(assertion.actual ?? "")}\`)`),
    "",
    "## Screenshots",
    ...report.evidence.screenshots.map((screenshot) => `![${screenshot}](${screenshot})`),
    "",
    "## Findings",
    ...(report.findings.length ? report.findings.map((finding) => `- **${finding.severity.toUpperCase()}** ${finding.title}: ${finding.actual}`) : ["- None"]),
    "",
  ];
  return `${lines.join("\n")}\n`;
}
