...
export async function runScenario(
      ...
      for (const [index, spec] of scenario.steps.entries()) {
        ...
        try {
          if (spec.action === 'goto') {
            ...
            assertionResult = await assertWithEvidence(
              driver,
              recorder,
              `assertion-${index + 1}`,
              spec.description,
            ...
            assertionResult.assertion.evidence = assertionResult.assertion.evidence.map((evidence) =>
              relative(runDir, evidence),
            )
          ...
          if (!assertionResult) {
            const frame = await recorder.capture(
              driver,
              spec.description ?? spec.action,
              'passed',
              'assertion',
              scenario.name,
            )
  ...
  const failed =
    fatalError ||
    assertions.some((assertion) => assertion.status !== 'pass') ||
    steps.some((step) => step.status !== 'pass')
